...
PROJECTION_FIELDS = os.environ.get('PROJECTION_FIELDS', 'id,severity,#datetime,message').split(',')
...
def validate_projection_fields():
    ...
    try:
        ...
        indexed_fields = {attr['AttributeName'] for attr in response['Table']['AttributeDefinitions']}
        projection_attributes = {field for field in PROJECTION_FIELDS if field not in EXPRESSION_ATTRIBUTE_NAMES}
        unindexed_fields = projection_attributes - indexed_fields
        if unindexed_fields:
            logger.warning(f"The following projection fields are not defined in the index schema: {unindexed_fields}")
...
def execute_query(index_name: str, key_condition, limit: int, start_key: Optional[Dict] = None) -> Tuple[list, bool, Optional[str]]:
    params = {
    ...
    try:
        ...
        metrics.add_metric(name=f"{index_name}QueryLatency", unit=MetricUnit.Milliseconds, value=latency)
        ...
        metrics.add_metric(name=f"{index_name}QueryErrors", unit=MetricUnit.Count, value=1)
        raise
    ...
    try:
        ...
        if 'startKey' in query_params and query_params['startKey']:
            try:
                ...
                if not validate_start_key(start_key, severity):
                    return build_response(400, {'error': 'Invalid pagination token structure'})
            except ValueError as e:
                return build_response(400, {'error': str(e)})

        logger.info("Processing log request", extra={
            'severity': severity,
            'limit': limit,
        ...
        if severity:
            if severity not in VALID_SEVERITIES:
                return build_response(400, {'error': f"Invalid severity. Must be one of {sorted(VALID_SEVERITIES)}"})
